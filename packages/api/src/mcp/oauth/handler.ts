...
export class MCPOAuthHandler {
  ...
  private static async discoverMetadata(
    ...
    if (!rawMetadata) {
       ...
       */
      logger.warn(
        `[MCPOAuth] No OAuth metadata discovered from ${sanitizeUrlForLogging(
          authServerUrl,
        )}, using legacy fallback endpoints`,
      );
  ...
  private static async registerOAuthClient(
    ...
    logger.debug(
      `[MCPOAuth] Starting client registration for ${sanitizeUrlForLogging(
        serverUrl,
      )}, server metadata:`,
      {
    ...
    if (supportedGrantTypes.includes('refresh_token')) {
      ...
      logger.debug(
        `[MCPOAuth] Server ${sanitizeUrlForLogging(
          serverUrl,
        )} does not support \`refresh_token\` grant type`,
      );
    }
  ...
  static async initiateOAuthFlow(
    ...
    logger.debug(
      `[MCPOAuth] initiateOAuthFlow called for ${serverName} with URL: ${sanitizeUrlForLogging(
        serverUrl,
      )}`,
    );
    ...
    try {
      ...
      if (config?.authorization_url && config?.token_url && config?.client_id) {
        ...
        logger.debug(
          `[MCPOAuth] Authorization URL generated: ${sanitizeUrlForLogging(
      ...
      logger.debug(
        `[MCPOAuth] Starting auto-discovery of OAuth metadata from ${sanitizeUrlForLogging(
          serverUrl,
        )}`,
      );
      ...
      logger.debug(
        `[MCPOAuth] OAuth metadata discovered, auth server URL: ${sanitizeUrlForLogging(
          authServerUrl,
        )}`,
      );
  ...
  static async completeOAuthFlow(
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `[MCPOAuth] Invalid resource URL format for flow ${flowId}: '${
  ...
  static async refreshOAuthTokens(
    ...
    try {
      ...
      if (metadata.clientInfo?.client_id) {
        ...
        logger.debug(
          `[MCPOAuth] Has client secret: ${!!metadata.clientInfo.client_secret} for ${
            metadata.serverName
          }`,
        );
  ...
  public static async revokeOAuthToken(
    ...
    logger.info(
      `[MCPOAuth] Revoking tokens for ${serverName} via ${sanitizeUrlForLogging(
        revokeUrl.toString(),
      )}`,
    );
